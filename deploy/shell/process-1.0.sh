...
LOG_FILE=log4j.properties                                  # 程序运行日志文件
...
function service_start()
{
    ...
    if [ "${status}" == "${STOP_STATUS}" ]; then
        ...
        # 3. 加载程序，启动程序
        nohup java -Xmx2048m -Xmx2048m -Xms256m -XX:+UseG1GC                             \
                   -Dlog4j.configuration=file:"${SERVICE_DIR}/conf/${LOG_FILE}"          \
                   -jar "${SERVICE_DIR}/${SERVICE_NAME}"                                 \
